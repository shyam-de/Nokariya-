    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-2 mb-6 flex gap-2">
          ...
          >
            📢 My Concerns ({myConcerns.filter((c: any) => c.status === 'PENDING').length > 0 ? myConcerns.filter((c: any) => c.status === 'PENDING').length : myConcerns.length})
          </button>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Labor Types & Workers <span className="text-red-500">*</span> (Add labor types with required count)
                  </label>
                  <div className="space-y-3 max-h-64 overflow-y-auto p-3 border-2 border-gray-300 rounded-lg">
                    {formData.laborTypeRequirements.map((req, index) => (
                      <div key={index} className="flex items-center gap-3 p-3 bg-primary-50 rounded-lg border border-primary-200">
                  ...
                  {formData.laborTypeRequirements.length === 0 && (
                    <p className="text-xs text-red-500 mt-1">Please add at least one labor type requirement</p>
                  )}
                  {formData.laborTypeRequirements.length > 0 && (
                    <div className="mt-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
                      <p className="text-sm text-blue-800">
                        <span className="font-semibold">Total Workers Required:</span> {
                    ...
                    <div className="mt-4 pt-4 border-t border-green-200">
                      ...
                      <div className="space-y-2">
                        {request.deployedWorkers.map((dw: any, idx: number) => {
                          ...
                          // Get rating from workerRating field (set by backend)
